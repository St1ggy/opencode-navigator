...
Пока я просто щёлкаю секции, изменения живут только в текущем процессе OpenCode и не сохраняются после перезапуска. Настройки для следующих запусков обновятся после `Save current layout as default`. `Restore configured layout` удаляет сохранённый layout, а `Restore configured behavior` возвращает MCP memory, LSP icons и оба shortcut к значениям из `tui.json`.
...
Текущая версия на момент публикации — `0.5.0`.
