...
function mcpToggleAction(status: string): 'connect' | 'disconnect' | undefined {
  if (status === 'pending') return

  return status === 'connected' ? 'disconnect' : 'connect'
}

export function layoutPresetPreview(
  current: { sections: SectionVisibility; expanded: SectionVisibility; order: SidebarSection[] },
  preset: SectionLayoutDefault,
) {
