...
export function SelectionBox(props: BoxProps & { iconOnly?: boolean }) {
  ...
  // Each cap occupies one cell; icon-only content adds 0 cells left and 1 right.
  ...
  function before(this: BoxRenderable, buffer: OptimizedBuffer, delta: number) {
    ...
    if (this.buffered || this.width < 3 || this.height < 1 || this.backgroundColor.a === 0) return
  ...
  return (
    <box
      ...
      paddingRight={props.paddingRight ?? props.padding ?? (local.iconOnly ? 2 : 1)}
