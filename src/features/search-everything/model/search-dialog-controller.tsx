import { type InputRenderable, RGBA, type Renderable, type ScrollBoxRenderable, parseColor } from '@opentui/core'
...
import { useDialogState, useDialogs, useIcons } from '../../../shared/ui'

import { SEARCH_GROUPS, type SearchGroup, buildSearchCandidates, searchContext, searchResults } from './search'
...
export function createSearchDialogController(props: SearchServices & { returnTarget?: Renderable | null }) {
  const context = searchContext(props.api)
  const skillTarget = props.skills.target()
  const mcpTarget = props.mcp.target()
  const [query, setQueryValue] = useDialogState('query', '')
    ...
    if (item.group === 'Skills' && props.preferences.shouldConfirmSkill(item.skill)) {
      dialogs.open(() => (
        <SkillDialog
          api={props.api}
          skill={item.skill}
          favorite={props.preferences.isFavoriteSkill?.(item.skill) ?? false}
          favoriteDisabled={props.preferences.ready ? !props.preferences.ready() : false}
          onToggleFavorite={() => props.preferences.toggleFavoriteSkill?.(item.skill)}
          onAccept={(skip) => {
            if (!sameContext()) return
...
export type SearchDialogController = ReturnType<typeof createSearchDialogController>
