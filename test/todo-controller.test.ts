...
test('a stale refresh does not overwrite a newer todo event', async () => {
  let resolveRequest!: (value: { data: { content: string; status: string; priority: string }[] }) => void
  const request = new Promise<{ data: { content: string; status: string; priority: string }[] }>((resolve) => {
    resolveRequest = resolve
  })
  let onTodoUpdated!: (event: {
    properties: { sessionID: string; todos: { content: string; status: string; priority: string }[] }
  }) => void
...
test('routes todo requests by their immutable session target and ignores late route success', async () => {
  const requests: {
    ...
    resolve: (value: { data: { content: string; status: string }[] }) => void
  }[] = []
    ...
    client: {
      session: {
        todo: (input: Record<string, unknown>) =>
          new Promise<{ data: { content: string; status: string }[] }>((resolve) => {
...
test('changing the active todo context eagerly invalidates an abort-ignoring request', async () => {
  let resolveOld!: (value: { data: { content: string; status: 'pending' }[] }) => void
    ...
    client: {
      session: {
        todo: ({ sessionID }: { sessionID: string }, options: { signal: AbortSignal }) => {
          ...
          return new Promise<{ data: { content: string; status: 'pending' }[] }>((resolve) => {
            resolveOld = resolve
          })
        },
      },
