...
test('skill insertion does not report success for a rejected or superseded append', async () => {
  ...
  const api = {
    ...
    client: { tui: { appendPrompt: () => (isPending ? response.promise : Promise.resolve({ data: false })) } },
    event: { on: () => () => {} },
...
test('preserves cached skills on failure and a retry wins over a late failure', async () => {
  type Result = { data: { name: string; location: string; content: string }[] }
  const responses: ReturnType<typeof deferred<Result>>[] = []
  const lifecycle = new AbortController()
  const api = {
    route: { current: { name: 'home' } },
    state: { path: { directory: '/repo' }, session: { get: () => {} } },
    client: {
      app: {
        skills: () => {
